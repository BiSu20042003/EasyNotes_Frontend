...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full z-50 bg-gradient-to-r from-sky-300 to-sky-400 shadow-md">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between md:flex">

          <div className="w-full flex items-center justify-between md:hidden">

            <button className="text-2xl font-bold text-blue-900">
              ☰
            </button>
            ...
            <div className="flex items-center space-x-4">
              ...
              {!currUser ? (
                <Link
                  to="/login"
                  className="text-white bg-green-600 rounded-full w-9 h-9 flex items-center justify-center hover:bg-green-700"
                >
                  🔑
                </Link>
              ) : (
                <button
                  onClick={handleLogout}
                  className="text-white bg-red-600 rounded-full w-9 h-9 flex items-center justify-center hover:bg-red-700"
                >
                  ⎋
                </button>
              )}
          ...
          <div className="hidden md:flex w-full items-center justify-between">
            ...
            <form
              onSubmit={handleSearchSubmit}
              className="flex items-center w-full max-w-md ml-6"
            >
              <input
                type="text"
                placeholder="Search Author"
            ...
            <div className="flex items-center space-x-4">
              {!currUser ? (
                <Link
                  to="/login"
                  className="h-10 flex items-center justify-center rounded-full bg-green-600 text-white px-4 font-bold hover:bg-green-700 transition"
                >
                  Login
                </Link>
              ) : (
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    onClick={handleLogout}
                    className="h-10 px-4 rounded-full bg-red-600 text-white font-bold hover:bg-red-700 transition"
                  >
                    Logout
                  </button>
                </div>
